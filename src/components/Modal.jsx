...
function Modal({
  ...
  return (
    <div
      className={`fixed inset-0 flex ${placement} ${bottomSheet ? 'p-0 sm:p-6' : 'p-4 sm:p-6'}`}
    ...
    >
      ...
      <div
        ...
        tabIndex={-1}
        className={`relative w-full max-h-full overflow-y-auto overscroll-y-contain border border-app-border bg-app-surface shadow-card outline-none backdrop-blur-xl transition-all duration-200 ease-out ${hidden} ${
          bottomSheet ? 'rounded-t-3xl sm:rounded-3xl' : 'rounded-3xl'
        } ${bottomSheet ? `sm:${maxWidth}` : maxWidth} ${className}`}
        style={bottomSheet ? { paddingBottom: 'env(safe-area-inset-bottom)' } : undefined}
      >
        {children}
      </div>
    </div>
